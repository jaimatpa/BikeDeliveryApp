<template lang="html">
<Page :title="!isMobile ? 'Load Items' : ''">
            ...
            <v-stepper-content step="2">
                <div>
                    ...
                    <v-data-table width="100%" block cols="12" xs="12" sm="12" md="12" xl="12" :headers="headers" :items="equipment" item-key="name" class="elevation-1 ma-1 mb-2">
                        <template v-slot:[`item.checkedLoaded`]="{ item }">
                            <v-simple-checkbox v-model="item.checkedLoaded" v-ripple @input="saveUpdate(item)" :disabled="true">
                    ...
                    <v-row style="margin-top: 20px;">
                        <v-col cols="12" xs="12" sm="12" md="6" xl="6">
                            <v-btn block depressed color="accent" @click.stop="goBack">
                                Back
                            </v-btn>
                        </v-col>
                        <v-col cols="12" xs="12" sm="12" md="6" xl="6">
    ...
    async code(value) {
      if(!this.scannedItems.includes(value)){
        ...
        try {
          ...
          this.showSuccess(`${response.data.Loaded} scanned successfully`)
          this.equipment.forEach(item=>{
            if(item.id == response.data.item_id){
              item.Loaded = response.data.Loaded;
              item.checkedLoaded = 1;
            }
          })
