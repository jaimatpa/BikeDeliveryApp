...
<PageForm>
    <!-- Login Form -->
    <FormLogin title="Bike Delivery App" :logo="bikeLogo" logoWidth="155" logoHeight="98" width="340" email outlined :busy="busy" :error="error" registerURL="/register" forgotURL="/forgotPassword" @submit="loginUser" :user="user">
        <template v-slot:beforeSubmit>
            <!-- Verification Error -->
            <v-alert v-model="verifyError" type="error" dense text dismissible icon="mdi-email-alert">
                Your email address has not been verified.
                <a href="#" @click="sendVerifyEmail">Resend email</a>
            </v-alert>
            <v-alert v-model="verifyEmailSent" type="success" dense text dismissible icon="mdi-email-alert">
        ...
        async loginUser(credentials) {
            try {
                ...
                if (response.status == 200) {
                    ...
                    this.$router.push("/");
                    this.showSuccess("Welcome to the Bike App!");
                }
            } catch (err) {
