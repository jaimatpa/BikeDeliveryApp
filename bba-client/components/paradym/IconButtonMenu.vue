...
<v-menu open-on-click bottom :left="left" offset-y transition="slide-y-transition" :content-class="isMobile ? 'mobile-menu' : ''">
    ...
    <v-list class="pa-0" v-for="(item, index) in items" :key="index">
        <template>
            <v-list-item :link="!item.hasOwnProperty('action')" :to="item.hasOwnProperty('action') ? undefined : item.to" @click="runAction(item)">
                <v-list-item-content>
                    <div class="d-flex">
                        <v-list-item-title>{{ item.title }}</v-list-item-title>
                        <div class="d-flex rectangular-icon" v-if="isMobile && item.iconImage">
                            <v-icon>{{$nuxt.$route.path != item.to ? item.iconImage : item.iconSelected}}</v-icon>
                            <!-- <v-img max-height="15" max-width="15" :src="item.downArrowIcon" class="arrow-icon"></v-img> -->
                        </div>
                    </div>
                </v-list-item-content>
