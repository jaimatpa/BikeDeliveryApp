...
<v-dialog class="qf-modal qf-modal-capture-photo" content-class="custom-class-photo-cap-modal" :value="show" @click:outside="$emit('cancel', { cross_btn_clicked: true })" light>
    <OverlayLoading :active="loading || !cameraReady" :message="loadingMessage">
        <CapturePhoto v-if="cameraMode === 'capture'" ref="camera" autoplay selectFirstDevice :images="capturedImages" @change-camera-mode="handleChangeCameraMode" @started="cameraReady = true" @onCapture="savePhoto($event)" v-on="$listeners" />
...
.custom-class-photo-cap-modal,
.custom-class-photo-cap-modal>form {
    margin: 0 !important;
    display: flex;
    align-self: flex-end;
}
