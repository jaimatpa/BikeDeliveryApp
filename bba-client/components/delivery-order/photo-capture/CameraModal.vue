<template>
  <v-dialog
    class="qf-modal qf-modal-capture-photo"
    content-class="custom-class-photo-cap-modal"
    :value="show"
    @click:outside="$emit('cancel', { cross_btn_clicked: true })"
    light
  >
  ...
  methods: {
    ...
    savePhoto(canvas) {
      canvas.toBlob(
        ...
        "image/jpeg",
        1
      ); // use .95 to get JPEG at 95% quality
