  ...
  try {
    ...
    if (userToLogin) {
      ...
      if (userToLogin.isVerified) {
        console.log('Login: User found.  Authenticating...')
        // Check the password
        ...
        if (isValidPassword) {
          ...
          console.log('Login: Invalid Password. Returning error.')
          return res.send({ 'error': true, 'type': 'password', 'message': 'Wrong password.  Try again.' })
        }
      } else {
        // User was not verified
        console.log('Login: User has not yet verified their email address.')
        return res.send({ 'error': true, 'type': 'verification', 'message': 'User not verified.' })
      }
    }
    else {
