...
function isValid(file, accept) {
  ...
  let isValid =
    valid.extensions.reduce(
      (prev, regex) => prev || !!file.originalname.match(regex),
      false
    ) ||
    valid.mimetypes.reduce(
      (prev, regex) => prev || !!file.mimetype.match(regex),
      false
    );
  return isValid;
}
...
function validatedAccept(accept) {
  if (accept) {
    return {
      extensions: accept.split(",").filter((type) => type.match(/^\.(?!.*\/)/)), // Get only extension filters
      mimetypes: accept
        .split(",")
        .filter((type) =>
          type.match(
